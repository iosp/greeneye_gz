    ...
    , f_addr(addr)
{
    ...
    f_socket = socket(AF_INET, SOCK_DGRAM , IPPROTO_UDP);
    if(f_socket == -1)
    {
        freeaddrinfo(f_addrinfo);
        throw udp_client_server_runtime_error(("could not create socket for: \"" + addr + ":" + decimal_port + "\"").c_str());
    }
    if (connect(f_socket, f_addrinfo->ai_addr, f_addrinfo->ai_addrlen) == -1)
    {
    	close(f_socket);
    	throw udp_client_server_runtime_error("Cannot connect");
    }
}
 ...
 */
udp_server::udp_server(const std::string& a_IpAddress, int a_port)
{
    f_port = a_port;
    f_addr = a_IpAddress;
    f_socket = 0;
...
/**
 * Connect to request endpoint
 */
int udp_server::Connect(bool IsBlocking)
{
	char decimal_port[16];
	...
	snprintf(decimal_port, sizeof(decimal_port), "%d", f_port);
	    ...
	    if(r != 0 || f_addrinfo == NULL)
	    {
	        throw udp_client_server_runtime_error(("invalid address or port for UDP socket: \"" + f_addr + ":" + decimal_port + "\"").c_str());
	        return -1;
	    }

	    // create endpoint for communication
// #Update 15/7/15 - DF: Set to nonblock
	    if (IsBlocking)
	    	f_socket = socket(f_addrinfo->ai_family, SOCK_DGRAM | SOCK_CLOEXEC, IPPROTO_UDP);
	    else
	    	f_socket = socket(f_addrinfo->ai_family, SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, IPPROTO_UDP);
	    if(f_socket == -1)
	    {
	        freeaddrinfo(f_addrinfo);
	        throw udp_client_server_runtime_error(("could not create UDP socket for: \"" + f_addr + ":" + decimal_port + "\"").c_str());
	        return -1;
	    }
	    ...
	    if(r != 0)
	    {
	        freeaddrinfo(f_addrinfo);
	        close(f_socket);
	        throw udp_client_server_runtime_error(("could not bind UDP socket with: \"" + f_addr + ":" + decimal_port + "\"").c_str());
	        return -1;
	    }
